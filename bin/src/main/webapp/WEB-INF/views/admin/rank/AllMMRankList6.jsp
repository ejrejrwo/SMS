<%@page import="com.boot.finalpro.betting.model.MMRRankingDTO"%>
...
<thead>
	...
	<tr>
		<th colspan="3">ALL <img alt="" src="../image/hello.png" style="width: 22px; height: 22px;"></th>
	</tr>
	<tr>
...
<tbody>
	<c:if test="${empty ranklist6 }">
	<tr>
		<td colspan="3" style="text-align: center;">랭킹 목록이 없습니다.</td>
	</tr>
	</c:if>
	
	<c:forEach items="${ranklist6 }" var="rank" varStatus="vs">	<!-- 시작 : 1 -->
	...
	<tr align="center" class="trfoot">
		...
		<c:if test="${gparam.prev }">
			<li class="page-item"><a class="page-link" href="MMRRankList6.do?keyword=${gparam.keyword }&pageNum=${gparam.startPage-1}">prev</a></li>
		</c:if>
		...
		<c:forEach begin="${gparam.startPage }" end="${gparam.endPage }" step="1" var="index">
			...
			<c:if test="${gparam.pageNum ne index }">
				<li class="page-item"><a class="page-link" href="MMRRankList6.do?keyword=${gparam.keyword }&pageNum=${index}">${index}</a></li>
			</c:if>
		</c:forEach>
		
		<c:if test="${gparam.next }">
			<li class="page-item"><a class="page-link" href="MMRRankList6.do?keyword=${gparam.keyword }&pageNum=${gparam.endPage+1}">next</a></li>
		</c:if>
		</ul>
		</td>
	</tr>
...
function search6() {
	$("#_frm6").attr("action", "MMRRankList6.do").submit();
}
